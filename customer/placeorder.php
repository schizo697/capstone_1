        ...
        while($productrow = mysqli_fetch_assoc($productres)){
            ...
            if($quantityres && mysqli_num_rows($quantityres) > 0){
                while($quantityrow = mysqli_fetch_assoc($quantityres)){
                    ...
                    $orders = "INSERT INTO orders (order_code, user_id, prodid, pname, price, quantity, date_of_order, total, status) VALUES ('$order_code', '$user_id', '$prod_id', '$prodname', '$prodprice', '$prodquantity', '$date_of_order', '$total', 1)";
                    $ordersres = mysqli_query($conn, $orders);
                    if ($ordersres) {
                        $oder_update = "UPDATE product SET quantity = quantity - $prodquantity WHERE prodid = '$prod_id'";
